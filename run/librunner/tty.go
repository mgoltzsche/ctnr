...
func setupIO(process *libcontainer.Process, container libcontainer.Container, createTTY, detach bool, sockpath string) (t *tty, err error) {
	defer func() {
		if err != nil {
			err = fmt.Errorf("setup process IO: %s", err)
		}
	}()
